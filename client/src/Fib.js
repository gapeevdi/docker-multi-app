import React, { Component } from 'react';
import axios from 'axios';

class FibNumbers extends Component{
    state = {
        calculatedIndexes: [],
        values : {},
        index: ''
    };

    componentDidMount(){
        this.fetchValues();
        this.fetchIndexes();
    }

    async fetchValues(){
        const values = await axios.get('/api/values/current');
        this.setState({values: values.data});
    }

    async fetchIndexes(){
        const calculatedIndexes = await axios.get('/api/values/all');
        this.setState({calculatedIndexes: calculatedIndexes.data});
    }

    renderCalculatedIndexes(){
        return this.state.seenIndexes.map(({ number }) => number.join(', '));
    }

    renderCalculatedValued(){
        const entries = [];

        for(let key in this.state.values){
            entries.push(
                <div key={key}>
                    For index {key} the system calculated {this.state.values[key]}
                </div>
            );
        }
    }

    handleSubmit = async (event) => {
        event.preventDefault();

        await axios.post('/api/values', {
            index: this.state.index
        });

        this.setState({index : ''});
    };

    render(){
        return (
            <div>
                <form onSubmit={this.handleSubmit}>
                    <label>Enter you index:</label>
                    <input value={this.state.index} onChange={event=>this.setState({index: event.target.value})}/>
                    <button>Submit</button>
                </form>

                <h3>Calculated indexes:</h3>
                {this.renderCalculatedIndexes()}

                <h3>Calculated values</h3>

            </div>
        );
    }
}

export default FibNumbers;